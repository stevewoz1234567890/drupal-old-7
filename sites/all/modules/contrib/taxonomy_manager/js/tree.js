/**
 * @files js for collapsible tree view with some helper functions for updating
 *   tree structure
 */
...
(function ($) {
  ...
  Drupal.TaxonomyManagerTree.prototype.attachTreeview = function (ul, currentIndex) {
    ...
    $(expandableParent).click(function () {
      ...
      // save all value of data into localstorage for html5
      var data_tid = $(li).data('li-tid');
      tree.toggleTree(li);
      localStorage.setItem('li-tid-' + data_tid, $(li).is('.collapsable, .lastCollapsable') ? 'collapsable' : 'expandable');

      $(this).toggleClass('hitArea-expandable', $(li).is('.expandable, .lastExpandable'))
      $(this).toggleClass('hitArea-collapsable', $(li).is('.collapsable, .lastCollapsable'))

    });

    $(expandableParent).parent("li.expandable, li.lastExpandable").each(function () {
      var $this = $(this);
      var data_tid = $this.data('li-tid');
      if (localStorage.getItem('li-tid-' + data_tid) == 'collapsable') {

        $("li[data-li-tid='" + data_tid + "'] >.hitArea").click();
      }
      else {
        $('div.hitArea', $this).addClass('hitArea-expandable');
  ...
  Drupal.TaxonomyManagerTree.prototype.loadRootForm = function (tids) {
    ...
    param['form_parents'] = this.formParents;
    param['language'] = this.language;
    param['terms_to_expand'] = tids; // can either be a single term id or
                                     // concatinated ids
  ...
  /**
   * adds link for loading next siblings terms, when click terms get loaded
   * through ahah adds all needed js like treeview, weightning, etc.. to new
   * added terms
   */
  ...
  Drupal.attachGlobalSelectAll = function () {
    $('span.taxonomy-manager-select-helpers').once(function () {
      ...
      //Remove custom key from Locastorage
      $('.remove-all-settings', $(this)).click(function () {
        $.each(localStorage, function (index, value) {
          var $key = localStorage.key(index)
          if ($key != undefined && $key.indexOf('li-tid-') != -1) {
      ...
      $('.expand-all', $(this)).click(function () {
        $('div.hitArea-expandable').click();
      });

      $('.collapse-all', $(this)).click(function () {
        $('div.hitArea-collapsable').click();
      });
